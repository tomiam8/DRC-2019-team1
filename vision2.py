...
def process_image(color_frame, thresh_yellow_low, thresh_yellow_high, thresh_blue_low, thresh_blue_high, debug):
    ...
    (contours_yellow, _) = cv2.findContours(edges_yellow, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
    ...
    (contours_blue, _) = cv2.findContours(edges_blue, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
    ...
    # Debug display stuff
    if debug:
        cv2.imshow("Threshold yellow_pic", threshold_yellow_img)
        cv2.imshow("Threshold blue pic", threshold_blue_img)
